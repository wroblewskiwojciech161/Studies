            ...
            while(p<neighbourLength){
                    if(neighbour[p][2]<best){
                        if(checkIfTabuSwap(neighbour[p][0], neighbour[p][1], tabuArray)==false){
                            ...
                            if(checkIfPathLeadsToDoor(convertFromCharToCoordinates(findStart(grid, sizeX, sizeY), current), grid, sizeX, sizeY).length>1){

                                best=checkIfPathLeadsToDoor(convertFromCharToCoordinates(findStart(grid, sizeX, sizeY), current), grid, sizeX, sizeY).length;  
